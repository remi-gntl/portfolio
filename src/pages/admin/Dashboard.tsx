      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-10 gap-4 border-b border-slate-800 pb-8">
          <div>
            ...
            <p className="text-gray-400 mt-1">Gérez le contenu de votre portfolio.</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <div className="bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden">
            <div className="p-6 border-b border-slate-800 flex justify-between items-center bg-slate-800/50">
              <h2 className="text-xl font-bold flex items-center gap-2">
                Mes Projets <span className="text-sm bg-blue-600 px-2 py-0.5 rounded-full">{projects.length}</span>
              </h2>
              <button 
            ...
            <div className="divide-y divide-slate-800 max-h-[500px] overflow-y-auto">
              {projects.length === 0 ? (
                <div className="p-8 text-center text-gray-500">Aucun projet pour l'instant.</div>
              ) : (
                projects.map((project) => (
                  <div key={project.id} className="p-4 flex items-center gap-4 hover:bg-slate-800/30 transition-colors group">
                    {/* Image miniature */}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex gap-2 mt-1">
                        {project.TechStack?.slice(0, 3).map((t: string) => (
                          <span key={t} className="text-[10px] bg-slate-800 px-1.5 py-0.5 rounded text-blue-300 border border-slate-700">{t}</span>
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button 
                        onClick={() => navigate(`/admin/projects/edit/${project.id}`)} // On créera cette route après
          ...
          <div className="bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden">
            <div className="p-6 border-b border-slate-800 flex justify-between items-center bg-slate-800/50">
              <h2 className="text-xl font-bold flex items-center gap-2">
                Mes Compétences <span className="text-sm bg-blue-600 px-2 py-0.5 rounded-full">{skills.length}</span>
              </h2>
              <button 
            ...
            <div className="divide-y divide-slate-800 max-h-[500px] overflow-y-auto">
              {skills.length === 0 ? (
                <div className="p-8 text-center text-gray-500">Aucune compétence.</div>
              ) : (
                skills.map((skill) => (
                  <div key={skill.id} className="p-4 flex items-center justify-between hover:bg-slate-800/30 transition-colors group">
